...
const int Value_Ru_SumNik = 5000;
...
const int Value_Ru_PierceArrow = 5000;
...
instance ItRu_PierceArrow ( C_ITEM )
{
    Name			= NAME_Rune; 					// Имя из файла Text.d "Руна"
    MainFlag		= ITEM_KAT_RUNE; 				// Тип Руна
    Flags			= 0;
    Value			= Value_Ru_PierceArrow; 							// Цена
    Visual			= "ItRu_Water04.3DS"; 			// Визуальный 3DS объект
    Material		= MAT_STONE;  					// Материал Камень
    Spell			= SPL_PierceArrow;  				// Наше заклинание
    Mag_Circle		= 7; 							// Круг магии можно убрать
    Wear			= WEAR_EFFECT;
    Effect			= "SPELLFX_WEAKGLIMMER_BLUE"; 	// Синее свечение руны
    Description		= NAME_SPL_PierceArrow; 			// Название нашего заклинания в файле Text.d
	
    Text [0]       	= NAME_Mag_Circle; 		Count [0] 	= Mag_Circle; 			// Круг
    Text [1]		= NAME_Manakosten;	    Count [1]	= SPL_Cost_PierceArrow; 	// Стоит маны
    Text [2]		= NAME_Damage; 			Count [2]	= SPL_DAMAGE_PierceArrow; 	// Повреждение
    Text [5]		= NAME_Value; 			Count [5] 	= Value;  				// Цена
};

instance ItRu_SumNik(C_Item)
{
	name = NAME_Rune;
	mainflag = ITEM_KAT_RUNE;
	flags = 0;
	value = Value_Ru_SumNik;
	visual = "ItRu_SumGol.3DS";
	material = MAT_STONE;
	spell = SPL_SummonNikita;
	mag_circle = 7;
	wear = WEAR_EFFECT;
	effect = "SPELLFX_WEAKGLIMMER_BLUE";
	description = NAME_SPL_SummonNikita;
	text[0] = NAME_Mag_Circle;
	count[0] = mag_circle;
	text[1] = NAME_Manakosten;
	count[1] = SPL_Cost_SummonNikita;
	text[5] = NAME_Value;
	count[5] = value;
};
