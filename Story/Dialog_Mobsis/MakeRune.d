...
instance PC_Circle_07(C_Info)
{
	npc = PC_Hero;
	condition = PC_Circle_07_Condition;
	information = PC_Circle_07_Info;
	permanent = TRUE;
	description = "Создать руну 7-го круга";
};


func int PC_Circle_07_Condition()
{
	if((PLAYER_MOBSI_PRODUCTION == MOBSI_MakeRune) && ((PLAYER_TALENT_RUNES[SPL_PierceArrow] == TRUE) || (PLAYER_TALENT_RUNES[SPL_SummonNikita] == TRUE)))
	{
		return TRUE;
	};
};

func void PC_Circle_07_Info()
{
	Info_ClearChoices(PC_Circle_07);
	Info_AddChoice(PC_Circle_07,Dialog_Back,PC_Circle_07_BACK);
	if(PLAYER_TALENT_RUNES[SPL_PierceArrow] == TRUE)
	{
		Info_AddChoice(PC_Circle_07,NAME_SPL_PierceArrow,PC_ItRu_PierceArrow_Info);
	};
	if(PLAYER_TALENT_RUNES[SPL_SummonNikita] == TRUE)
	{
		Info_AddChoice(PC_Circle_07,NAME_SPL_SummonNikita,PC_ItRu_SumNik_Info);
	};
};

func void PC_Circle_07_BACK()
{
	Info_ClearChoices(PC_Circle_07);
};
...
func void PC_ItRu_PierceArrow_Info()
{
	if((Npc_HasItems(hero,ItSc_PierceArrow) >= 1) && (Npc_HasItems(hero,ItMi_Pitch) >= 1) && (Npc_HasItems(hero,ItMi_Sulfur) >= 1) && (Npc_HasItems(hero,ItAt_WaranFiretongue) >= 3))
	{
		Npc_RemoveInvItems(hero,ItSc_PierceArrow,1);
		Npc_RemoveInvItems(hero,ItMi_Pitch,1);
		Npc_RemoveInvItems(hero,ItAt_WaranFiretongue,3);
		CreateInvItems(hero,ItRu_PierceArrow,1);
		Print(PRINT_RuneSuccess);
	}
	else
	{
...
func void PC_ItRu_SumNik_Info()
{
	if((Npc_HasItems(hero,ItSc_SumNik) >= 1) && (Npc_HasItems(hero,ItMw_Nikita_Piercer) >= 1))
	{
		Npc_RemoveInvItems(hero,ItSc_SumNik,1);
		Npc_RemoveInvItems(hero,ItMw_Nikita_Piercer,1);
		CreateInvItems(hero,ItRu_SumNik,1);
		Print(PRINT_RuneSuccess);
	}
	else
	{
